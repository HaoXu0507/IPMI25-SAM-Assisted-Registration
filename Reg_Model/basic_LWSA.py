...
class SwinTransformerBlock(nn.Module):
    ...
    """
    # 参数中值得注意的是depths和num_heads, len(depths)
    # 作为Swin中的stage数，而depths中的每一个值代表每一个stage中有多少个Swin
    # Transformer
    # Block。需要注意的是一个block中只会有一个MSA，就是说W - MSA和SW - MSA必然存在于不同的Block。num_heads指的是在每一个stage中的所有Block中的MSA使用的头数，由默认值我们可以看到第一个stage中的MSA的头数是3个。checkpoint如果使用可以节省空间。
