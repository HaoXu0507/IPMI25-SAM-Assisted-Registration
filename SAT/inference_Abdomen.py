...
def inference(model, save_dir, tokenizer, text_encoder, device, testloader, gaussian_filter):
    ...
    mask_label = ['background',
                  ...
                  "inferior vena cava","portal vein and splenic vein","pancreas","right adrenal gland","left adrenal gland"]


    text_label =["spleen","right kidney","left kidney","gallbladder", "esophagus","liver", "stomach", "aorta",
                "inferior vena cava","portal vein and splenic vein","pancreas","right adrenal gland","left adrenal gland",
               ]
    with torch.no_grad():
        ...
        for batch in testloader:    # one batch for each sample
            ...
            for prompts, n1n2 in zip(split_prompt, split_n1n2):
                ...
                for patches, y1y2_x1x2_z1z2_ls in zip(batched_patch, batched_y1y2x1x2z1z2):   # [b, c, h, w, d] 
                    ...
                    prediction_patch,embedding_patch = model(queries=batched_queries, image_input=patches)
                    # print(prediction_patch.size(),embedding_patch.size())
                    prediction_patch = torch.sigmoid(prediction_patch)
                    prediction_patch = prediction_patch.detach().cpu().numpy()  # bnhwd
                    embedding_patch = embedding_patch.detach().cpu().numpy()
                    # fill in 
                    for b in range(len(y1y2_x1x2_z1z2_ls)):
                        ...
                        embedding[n1:n2, y1:y2, x1:x2, z1:z2] += embedding_patch[b, :n2-n1, :y2-y1, :x2-x1, :z2-z1] * windows[:y2-y1, :x2-x1, :z2-z1]
            ...
            for j, label in enumerate(labels):
                ...
                smooth = 1e-5
                dice_3d = (2. * intersection ) / (np.sum(mask_j) + np.sum(seg_result) + smooth)
            ...
            # save emb
            np_emb = np.transpose(np_emb, (1, 2, 3, 0))
            emb_obj = nib.nifti2.Nifti1Image(np_emb, np.eye(4))
            nib.save(emb_obj, f'{save_dir}/{save_name}/emb.nii.gz')
...
if __name__ == '__main__':
    '''
    --data_jsonl
    /home/hao/PycharmProjects/SAT-main/Abdomen_CT_CT_data.jsonl
    --checkpoint
    /media/hao/DATA/download/SAT_Nano.pth
    --text_encoder_checkpoint
    /media/hao/DATA/download/Text_Encoder.pth
    --save_dir
    /media/hao/DATA/dataset/SAT/Nano/Abdomen/
    '''    
    ...
    parser.add_argument(
        "--save_dir",
        type=str,
        default='/media/hao/DATA/ckpt/SAT/AbdomenCTCT/',
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
